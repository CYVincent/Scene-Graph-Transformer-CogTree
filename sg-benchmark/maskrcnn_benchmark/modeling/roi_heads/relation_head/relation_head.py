    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if not loss_relation:
            ...
            if self.cfg.MODEL.ATTRIBUTE_ON and isinstance(loss_refine, (list, tuple)):
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.LOSS.USE_CLASS_BALANCED_LOSS or \
                    self.cfg.MODEL.ROI_RELATION_HEAD.LOSS.USE_FOCAL_LOSS or self.cfg.MODEL.ROI_RELATION_HEAD.LOSS.USE_RW_LOSS:
                    output_losses = dict(loss_refine_obj=loss_refine)
                else:
