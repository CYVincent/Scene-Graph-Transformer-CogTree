...
def get_parser():
    ...
    parser.add_argument('--checkpoint', type=str,
        ...
        ' are used for clustering.', default='/home/data1/yjgroup/cy/SGBenchmark/sg_benchmark/checkpoints4/REL_TSF-sgcls-CB/model_0025000.pth')
...
def _get_depth(G, node):
    if not G.succ[node]:
        return 1
    return max([_get_depth(G, child) for child in G.succ[node]]) + 1

def get_depth(G):

    return max([_get_depth(G, root) for root in get_roots(G)])
...
MODEL_FC_KEYS = (
    ...
    'roi_heads.relation.predictor.rel_compress.weight', 'module.roi_heads.relation.predictor.rel_compress.weight')
...
def build_induced_graph(wnids, checkpoint, model=None, linkage='ward',
    ...
    assert checkpoint or model or state_dict, \
        'Need to specify either `checkpoint` or `model` or `state_dict`.'
    if state_dict:
...
def get_centers_from_checkpoint(checkpoint):
    ...
    assert fc is not None, (
        f'Could not find FC weights in checkpoint {checkpoint} with keys: {data.keys()}')
    return fc
